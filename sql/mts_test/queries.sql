/*
	...
	В области "Решение" ниже напишите следующие запросы к базе данных prd_sbx_general
	...
	Для регионов и тарифных групп в результирующей таблице должны быть их названия

*/


-- Количество абонентов в разрезе месяцев
...
-- Количество абонентов в разрезе регионов
...
-- Количество абонентов в разрезе тарифов
...
-- Доля абонентов без начислений в разрезе месяцев
...
-- Доля абонентов без начислений в разрезе групп
WITH ptc (tbm, app_n, revenue, abon_rev) AS
  (SELECT *
   FROM pe_test_clc
   WHERE revenue = 0)
...
-- Доля абонентов без начислений в разрезе регионов
WITH ptc (tbm, app_n, revenue, abon_rev) AS
  (SELECT *
   FROM pe_test_clc
   WHERE revenue = 0)
...
-- Количество абонентов, активированных в отчётный месяц в разрезе регионов
SELECT ptr.region_name AS Regions,
       COUNT(*) AS Amount
FROM pe_test_abnt pta
